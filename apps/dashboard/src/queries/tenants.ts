...
function buildOrderBy(sort: string, dir: 'asc' | 'desc') {
  ...
  switch (sort) {
    case 'name':
      return fn(tenants.name)
    case 'status':
      return fn(tenants.status)
    case 'plan':
      return fn(tenants.plan)
    case 'subscription_status':
      return fn(tenants.subscription_status)
    default:
      return fn(tenants.created_at)
  }
}
...
export async function getTenantStats(tenantId: string): Promise<TenantStats> {
  const [productRes, sessionRes, orderRes, revenueRes] = await Promise.all([
    ...
    tenantDb.select({ c: count() }).from(sessions).where(eq(sessions.tenant_id, tenantId)),

    tenantDb.select({ c: count() }).from(orders).where(eq(orders.tenant_id, tenantId)),
...
export async function updateTenantBotConfig(slug: string, patch: Record<string, unknown>) {
  const row = await db.query.tenants.findFirst({ where: eq(tenants.slug, slug) })
  if (!row) return undefined
  const merged = { ...(row.bot_config as Record<string, unknown>), ...patch }
  const [updated] = await db
    .update(tenants)
    .set({ bot_config: merged, updated_at: new Date() })
    .where(eq(tenants.slug, slug))
