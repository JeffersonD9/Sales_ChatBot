...
const { formatPrice, formatPhone } = require('@whatsapp-saas/shared-utils');
...
async function notifySale(clientPhone, orderData, tenant) {
  ...
  const msg =
    ...
    `_Notificaci\u00f3n autom\u00e1tica \u2014 Bot Jest Solution_`;
...
async function notifyAdminBilling(event, tenant, extra = {}) {
  ...
  try {
    await transporter.sendMail({
      from:    `"Jest Solution Billing" <${process.env.SMTP_USER}>`,
      to:      adminEmail,
      subject,
      html:    body,
    });
